/**
 * GMCLIB_SpreadPWM_SF.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         uint16 y1  = GMCLIB_SpreadPWM_w(GMCLIB_SPREADPWM_T work1[1])
 *
...
#define S_FUNCTION_NAME  GMCLIB_SpreadPWM_SF
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GMCLIB_SPREADPWM_T", &dataTypeIdReg);
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GMCLIB_SpreadPWMInit_w", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GMCLIB_SpreadPWM_w", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GMCLIB_SpreadPWMInit_w_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GMCLIB_SpreadPWM_w_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_SpreadPWM_SF_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_SpreadPWM_SF_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        DTypeId __GMCLIB_SPREADPWM_TId = ssGetDataTypeId(S, "GMCLIB_SPREADPWM_T");
        ...
        __dtSizeInfo[0] = ssGetDataTypeSize(S, __GMCLIB_SPREADPWM_TId);
        ...
        /* Get information for accessing GMCLIB_SPREADPWM_T.u32Rnd */
        __dtBusInfo[0] = ssGetBusElementOffset(S, __GMCLIB_SPREADPWM_TId, 0);
        __dtBusInfo[1] = __dtSizeInfo[1];

        /* Get information for accessing GMCLIB_SPREADPWM_T.u16ModMin */
        __dtBusInfo[2] = ssGetBusElementOffset(S, __GMCLIB_SPREADPWM_TId, 1);
        __dtBusInfo[3] = __dtSizeInfo[2];

        /* Get information for accessing GMCLIB_SPREADPWM_T.u16ModMax */
        __dtBusInfo[4] = ssGetBusElementOffset(S, __GMCLIB_SPREADPWM_TId, 2);
        __dtBusInfo[5] = __dtSizeInfo[2];

        /* Get information for accessing GMCLIB_SPREADPWM_T.u16CntK1 */
        __dtBusInfo[6] = ssGetBusElementOffset(S, __GMCLIB_SPREADPWM_TId, 3);
        __dtBusInfo[7] = __dtSizeInfo[2];

        /* Get information for accessing GMCLIB_SPREADPWM_T.f16Inc */
        __dtBusInfo[8] = ssGetBusElementOffset(S, __GMCLIB_SPREADPWM_TId, 4);
        __dtBusInfo[9] = __dtSizeInfo[3];

        /* Get information for accessing GMCLIB_SPREADPWM_T.u16DevConst */
        __dtBusInfo[10] = ssGetBusElementOffset(S, __GMCLIB_SPREADPWM_TId, 5);
        __dtBusInfo[11] = __dtSizeInfo[2];


        /* Alloc memory for the pwork 1 (__work1BUS) */
        {
            GMCLIB_SPREADPWM_T* __work1BUS = (GMCLIB_SPREADPWM_T*)calloc(sizeof(GMCLIB_SPREADPWM_T), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GMCLIB_SPREADPWM_T* __work1BUS = (GMCLIB_SPREADPWM_T*) ssGetPWorkValue(S, 0);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].u32Rnd, work1 +  __dtBusInfo[0],  __dtBusInfo[1]);
        (void) memcpy(&__work1BUS[0].u16ModMin, work1 +  __dtBusInfo[2],  __dtBusInfo[3]);
        (void) memcpy(&__work1BUS[0].u16ModMax, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__work1BUS[0].u16CntK1, work1 +  __dtBusInfo[6],  __dtBusInfo[7]);
        (void) memcpy(&__work1BUS[0].f16Inc, work1 +  __dtBusInfo[8],  __dtBusInfo[9]);
        (void) memcpy(&__work1BUS[0].u16DevConst, work1 +  __dtBusInfo[10],  __dtBusInfo[11]);

        /* Call the legacy code function */
        GMCLIB_SpreadPWMInit_w(*p1, *p2, *p3, *p4, __work1BUS);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[0], &__work1BUS[0].u32Rnd,  __dtBusInfo[1]);
        (void) memcpy(work1 +  __dtBusInfo[2], &__work1BUS[0].u16ModMin,  __dtBusInfo[3]);
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].u16ModMax,  __dtBusInfo[5]);
        (void) memcpy(work1 +  __dtBusInfo[6], &__work1BUS[0].u16CntK1,  __dtBusInfo[7]);
        (void) memcpy(work1 +  __dtBusInfo[8], &__work1BUS[0].f16Inc,  __dtBusInfo[9]);
        (void) memcpy(work1 +  __dtBusInfo[10], &__work1BUS[0].u16DevConst,  __dtBusInfo[11]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        *y1 = GMCLIB_SpreadPWM_w(__work1BUS);
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    /* Verify Bus/StructType 'GMCLIB_SPREADPWM_T', specification */
    {
        uint32_T chk[] = {3602443752, 3403716842, 3281985364, 813783156};
        int_T status;
        status = CheckDataTypeChecksum(S, "GMCLIB_SPREADPWM_T", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'GMCLIB_SPREADPWM_T'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'GMCLIB_SPREADPWM_T' definition has changed since the S-Function was generated");
        }
    }
}
