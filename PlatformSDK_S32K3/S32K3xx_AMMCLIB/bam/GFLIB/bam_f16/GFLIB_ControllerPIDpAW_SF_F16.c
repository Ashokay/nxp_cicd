 ...
 *    C source code generated on : 12-Sep-2023 09:21:32
 *
 */
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        double upperLimitf;
        double lowerLimitf;
        double f16UpperLimit_tmp;
        double f16LowerLimit_tmp;
        double exparg;
        double expconst;
        double f16FiltCoef_tmp;
        double kdf;
        double s16DerivGainShift_tmp;
        double f16DerivGain_tmp;
        ...
        if((*p20) == 1) {
          ...
          *p7=(int16_T)s16PropGainShift_tmp;
          if((*p7)<0) {
            f16PropGain_tmp=kpf*32768.0*(double)(1UL << (-(*p7)));
          } else {
            f16PropGain_tmp=kpf*32768.0/(double)(1UL << (*p7));
          }
          ...
          *p8=(int16_T)s16IntegGainShift_tmp;
          if((*p8)<0) {
            f16IntegGain_tmp=kif*32768.0*(double)(1UL << (-(*p8)));
          } else {
            f16IntegGain_tmp=kif*32768.0/(double)(1UL << (*p8));
          }
          ...
          *p4=(int16_T)f16IntegGain_tmp;
          upperLimitf=(*p17)/(*p19);
          lowerLimitf=(*p16)/(*p19);
          f16UpperLimit_tmp=upperLimitf*32768.0;
          if(f16UpperLimit_tmp>32767.0) {
            f16UpperLimit_tmp=32767.0;
          }
          if(f16UpperLimit_tmp<-32768.0) {
            f16UpperLimit_tmp=-32768.0;
          }
          *p11=(int16_T)f16UpperLimit_tmp;
          f16LowerLimit_tmp=lowerLimitf*32768.0;
          if(f16LowerLimit_tmp>32767.0) {
            f16LowerLimit_tmp=32767.0;
          }
          if(f16LowerLimit_tmp<-32768.0) {
            f16LowerLimit_tmp=-32768.0;
          }
          *p10=(int16_T)f16LowerLimit_tmp;
          exparg=-SampleTime*(*p15);
          expconst=exp(exparg);
          f16FiltCoef_tmp=expconst*32768.0;
          if(f16FiltCoef_tmp>32767.0) {
            f16FiltCoef_tmp=32767.0;
          }
          if(f16FiltCoef_tmp<-32768.0) {
            f16FiltCoef_tmp=-32768.0;
          }
          *p6=(int16_T)f16FiltCoef_tmp;
          kdf=(*p14)/SampleTime*(1 - expconst)*(*p18)/(*p19);
          s16DerivGainShift_tmp=ceil(M_LOG2E*log(fabs((double)kpf)));
          if(s16DerivGainShift_tmp<-15) {
            s16DerivGainShift_tmp=-15;
          }
          if(s16DerivGainShift_tmp>15) {
            s16DerivGainShift_tmp=15;
          }
          *p9=(int16_T)s16DerivGainShift_tmp;
          if((*p9)<0) {
            f16DerivGain_tmp=kpf*32768.0*(double)(1UL << (-(*p9)));
          } else {
            f16DerivGain_tmp=kpf*32768.0/(double)(1UL << (*p9));
          }
          if(f16DerivGain_tmp>32767.0) {
            f16DerivGain_tmp=32767.0;
          }
          if(f16DerivGain_tmp<-32768.0) {
            f16DerivGain_tmp=-32768.0;
          }
          *p5=(int16_T)f16DerivGain_tmp;
        }
        GFLIB_ControllerPIDpAWInit_w_F16(*p3, *p4, *p5, *p6, *p7, *p8, *p9, *p10, *p11, __work1BUS, work2, *p2, *p12, *p13, *p14, *p15, *p16, *p17, *p18, *p19, *p20);
