...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        double cc1f;
        double cc2f;
        double u16NShiftPI_tmp;
        double u16NShiftPI_tmp2;
        double f32CC1sc_tmp;
        double f32CC2sc_tmp;
        double c1f;
        double u16NShiftI_tmp;
        double f32C1_tmp;
        ...
        if((*p16) == 1) {
          adcsc=(*p14)/(*p15);
          f32ADCMaxError_tmp=adcsc*2147483648.0;
          if(f32ADCMaxError_tmp>2147483647.0) {
            f32ADCMaxError_tmp=2147483647.0;
          }
          if(f32ADCMaxError_tmp<-2147483648.0) {
            f32ADCMaxError_tmp=-2147483648.0;
          }
          *p2=(int32_T)f32ADCMaxError_tmp;
          pgain=12.56637061435917*(*p11)*(*p10);
          igain=39.47841760435743*(*p10)*(*p10);
          cc1=pgain+igain*SampleTime*0.5;
          cc2=-pgain+igain*SampleTime*0.5;
          wmax=(*p13)*(*p12)*0.1047197551196598;
          cc1f=cc1*3.141592653589793/wmax;
          cc2f=cc2*3.141592653589793/wmax;
          u16NShiftPI_tmp=ceil(M_LOG2E*log(fabs((double)cc1f)));
          u16NShiftPI_tmp2=ceil(M_LOG2E*log(fabs((double)cc2f)));
          if(u16NShiftPI_tmp<u16NShiftPI_tmp2) {
            u16NShiftPI_tmp=u16NShiftPI_tmp2;
          }
          if(u16NShiftPI_tmp<0) {
            u16NShiftPI_tmp=0;
          }
          if(u16NShiftPI_tmp>31) {
            u16NShiftPI_tmp=31;
          }
          *p7=(uint16_T)u16NShiftPI_tmp;
          f32CC1sc_tmp=cc1f*2147483648.0/(double)(1UL << (*p7));
          if(f32CC1sc_tmp>2147483647.0) {
            f32CC1sc_tmp=2147483647.0;
          }
          if(f32CC1sc_tmp<-2147483648.0) {
            f32CC1sc_tmp=-2147483648.0;
          }
          *p3=(int32_T)f32CC1sc_tmp;
          f32CC2sc_tmp=cc2f*2147483648.0/(double)(1UL << (*p7));
          if(f32CC2sc_tmp>2147483647.0) {
            f32CC2sc_tmp=2147483647.0;
          }
          if(f32CC2sc_tmp<-2147483648.0) {
            f32CC2sc_tmp=-2147483648.0;
          }
          *p4=(int32_T)f32CC2sc_tmp;
          *p6=-2147483648;
          *p5=2147483647;
          c1f=SampleTime*0.5*wmax/3.141592653589793;
          u16NShiftI_tmp=ceil(M_LOG2E*log(fabs((double)c1f)));
          if(u16NShiftI_tmp<0) {
            u16NShiftI_tmp=0;
          }
          if(u16NShiftI_tmp>31) {
            u16NShiftI_tmp=31;
          }
          *p9=(uint16_T)u16NShiftI_tmp;
          f32C1_tmp=c1f*2147483648.0/(double)(1UL << (*p9));
          if(f32C1_tmp>2147483647.0) {
            f32C1_tmp=2147483647.0;
          }
          if(f32C1_tmp<-2147483648.0) {
            f32C1_tmp=-2147483648.0;
          }
          *p8=(int32_T)f32C1_tmp;
        }
        AMCLIB_WindmillingInit_w_F32(__work1BUS, *p2, *p3, *p4, *p5, *p6, *p7, *p8, *p9, *p10, *p11, *p12, *p13, *p14, *p15, *p16);
