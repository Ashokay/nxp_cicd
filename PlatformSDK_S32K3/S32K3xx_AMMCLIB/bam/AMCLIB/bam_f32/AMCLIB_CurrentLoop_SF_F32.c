...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        double cc1fq;
        double cc2fq;
        double u16NShift_D_tmp;
        double u16NShift_D_tmp2;
        double u16NShift_Q_tmp;
        double u16NShift_Q_tmp2;
        double f32CC1sc_D_tmp;
        double f32CC2sc_D_tmp;
        double f32CC1sc_Q_tmp;
        double f32CC2sc_Q_tmp;
        ...
        if((*p17) == 1) {
          w0=6.283185307179586*(*p10);
          pgainD=2.0*(*p11)*w0*(*p12)-(*p14);
          igainD=w0*w0*(*p12);
          pgainQ=2.0*(*p11)*w0*(*p13)-(*p14);
          igainQ=w0*w0*(*p13);
          if((*p9) == 1) {
            cc1d=pgainD;
            cc2d=-pgainD+igainD*SampleTime;
            cc1q=pgainQ;
            cc2q=-pgainQ+igainQ*SampleTime;
          } else {
            if((*p9) == 2) {
              cc1d=pgainD+igainD*SampleTime;
              cc2d=-pgainD;
              cc1q=pgainQ+igainQ*SampleTime;
              cc2q=-pgainQ;
            } else {
              cc1d=pgainD+igainD*SampleTime*0.5;
              cc2d=-pgainD+igainD*SampleTime*0.5;
              cc1q=pgainQ+igainQ*SampleTime*0.5;
              cc2q=-pgainQ+igainQ*SampleTime*0.5;
            }
          }
          cc1fd=cc1d*(*p16)/(*p15);
          cc2fd=cc2d*(*p16)/(*p15);
          cc1fq=cc1q*(*p16)/(*p15);
          cc2fq=cc2q*(*p16)/(*p15);
          u16NShift_D_tmp=ceil(M_LOG2E*log(fabs((double)cc1fd)));
          u16NShift_D_tmp2=ceil(M_LOG2E*log(fabs((double)cc2fd)));
          if(u16NShift_D_tmp<u16NShift_D_tmp2) {
            u16NShift_D_tmp=u16NShift_D_tmp2;
          }
          if(u16NShift_D_tmp<0) {
            u16NShift_D_tmp=0;
          }
          if(u16NShift_D_tmp>31) {
            u16NShift_D_tmp=31;
          }
          *p5=(uint16_T)u16NShift_D_tmp;
          u16NShift_Q_tmp=ceil(M_LOG2E*log(fabs((double)cc1fq)));
          u16NShift_Q_tmp2=ceil(M_LOG2E*log(fabs((double)cc2fq)));
          if(u16NShift_Q_tmp<u16NShift_Q_tmp2) {
            u16NShift_Q_tmp=u16NShift_Q_tmp2;
          }
          if(u16NShift_Q_tmp<0) {
            u16NShift_Q_tmp=0;
          }
          if(u16NShift_Q_tmp>31) {
            u16NShift_Q_tmp=31;
          }
          *p8=(uint16_T)u16NShift_Q_tmp;
          f32CC1sc_D_tmp=cc1fd*2147483648.0/(double)(1UL << (*p5));
          ...
          *p3=(int32_T)f32CC1sc_D_tmp;
          f32CC2sc_D_tmp=cc2fd*2147483648.0/(double)(1UL << (*p5));
          ...
          *p4=(int32_T)f32CC2sc_D_tmp;
          f32CC1sc_Q_tmp=cc1fq*2147483648.0/(double)(1UL << (*p8));
          if(f32CC1sc_Q_tmp>2147483647.0) {
            f32CC1sc_Q_tmp=2147483647.0;
          }
          if(f32CC1sc_Q_tmp<-2147483648.0) {
            f32CC1sc_Q_tmp=-2147483648.0;
          }
          *p6=(int32_T)f32CC1sc_Q_tmp;
          f32CC2sc_Q_tmp=cc2fq*2147483648.0/(double)(1UL << (*p8));
          if(f32CC2sc_Q_tmp>2147483647.0) {
            f32CC2sc_Q_tmp=2147483647.0;
          }
          if(f32CC2sc_Q_tmp<-2147483648.0) {
            f32CC2sc_Q_tmp=-2147483648.0;
          }
          *p7=(int32_T)f32CC2sc_Q_tmp;
        }
        AMCLIB_CurrentLoopInit_w_F32(__work1BUS, __work2BUS, work3, *p3, *p4, *p5, *p6, *p7, *p8, *p2, *p9, *p10, *p11, *p12, *p13, *p14, *p15, *p16, *p17);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        AMCLIB_CurrentLoop_w_F32(*u1, __work1BUS, __work2BUS, __u2BUS, __u3BUS, work3, *u4, *u5, *u6, *u7, *u8, *u9, *u10, *u11, *u12, *u13, *u14, *u15, __y1BUS, y2, y3, y4, y5, y6, *p1, *p2);
