/**
 * AMCLIB_CurrentLoopPIp_SF_FLT.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         void AMCLIB_CurrentLoopPIp_w_FLT(single u1, GFLIB_CONTROLLER_PIAW_P_T_FLT work1[1], GFLIB_CONTROLLER_PIAW_P_T_FLT work2[1], SWLIBS_2Syst_FLT u2[1], SWLIBS_2Syst_FLT u3[1], single u4, single u5, single u6, single u7, single u8, single u9, single u10, single u11, single u12, single u13, SWLIBS_2Syst_FLT y1[1], single y2[1], single y3[1], single y4[1], single y5[1], single y6[1], int32 work3[1], int32 p1, int32 p2)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 12-Sep-2023 09:21:22
 *
 */
...
#define S_FUNCTION_NAME  AMCLIB_CurrentLoopPIp_SF_FLT
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "AMCLIB_CurrentLoopPIpInit_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "AMCLIB_CurrentLoopPIp_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "AMCLIB_CurrentLoopPIpInit_w_FLT_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "AMCLIB_CurrentLoopPIp_w_FLT_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "AMCLIB_CurrentLoopPIp_SF_FLT_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "AMCLIB_CurrentLoopPIp_SF_FLT_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        __dtSizeInfo[2] = ssGetDataTypeSize(S, __GFLIB_CONTROLLER_PIAW_P_T_FLTId);
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GFLIB_CONTROLLER_PIAW_P_T_FLT* __work1BUS = (GFLIB_CONTROLLER_PIAW_P_T_FLT*) ssGetPWorkValue(S, 3);
        GFLIB_CONTROLLER_PIAW_P_T_FLT* __work2BUS = (GFLIB_CONTROLLER_PIAW_P_T_FLT*) ssGetPWorkValue(S, 4);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].fltPropGain, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__work1BUS[0].fltIntegGain, work1 +  __dtBusInfo[6],  __dtBusInfo[7]);
        ...
        (void) memcpy(&__work1BUS[0].fltInK_1, work1 +  __dtBusInfo[14],  __dtBusInfo[15]);
        ...
        (void) memcpy(&__work2BUS[0].fltPropGain, work2 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__work2BUS[0].fltIntegGain, work2 +  __dtBusInfo[6],  __dtBusInfo[7]);
        (void) memcpy(&__work2BUS[0].fltLowerLimit, work2 +  __dtBusInfo[8],  __dtBusInfo[9]);
        (void) memcpy(&__work2BUS[0].fltUpperLimit, work2 +  __dtBusInfo[10],  __dtBusInfo[11]);
        (void) memcpy(&__work2BUS[0].fltIntegPartK_1, work2 +  __dtBusInfo[12],  __dtBusInfo[13]);
        (void) memcpy(&__work2BUS[0].fltInK_1, work2 +  __dtBusInfo[14],  __dtBusInfo[15]);
        (void) memcpy(&__work2BUS[0].u16LimitFlag, work2 +  __dtBusInfo[16],  __dtBusInfo[17]);

        /* Call the legacy code function */
        if((*p12) == 1) {
          w0=6.283185307179586*(*p7);
          pgainD=2.0*(*p8)*w0*(*p9)-(*p11);
          igainD=w0*w0*(*p9);
          pgainQ=2.0*(*p8)*w0*(*p10)-(*p11);
          igainQ=w0*w0*(*p10);
          *p3=pgainD;
          *p4=igainD*SampleTime*0.5;
          *p5=pgainQ;
          *p6=igainQ*SampleTime*0.5;
        }
        AMCLIB_CurrentLoopPIpInit_w_FLT(__work1BUS, __work2BUS, *p3, *p4, *p5, *p6, work3, *p2, *p7, *p8, *p9, *p10, *p11, *p12);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].fltPropGain,  __dtBusInfo[5]);
        (void) memcpy(work1 +  __dtBusInfo[6], &__work1BUS[0].fltIntegGain,  __dtBusInfo[7]);
        ...
        (void) memcpy(work1 +  __dtBusInfo[14], &__work1BUS[0].fltInK_1,  __dtBusInfo[15]);
        ...
        (void) memcpy(work2 +  __dtBusInfo[4], &__work2BUS[0].fltPropGain,  __dtBusInfo[5]);
        (void) memcpy(work2 +  __dtBusInfo[6], &__work2BUS[0].fltIntegGain,  __dtBusInfo[7]);
        (void) memcpy(work2 +  __dtBusInfo[8], &__work2BUS[0].fltLowerLimit,  __dtBusInfo[9]);
        (void) memcpy(work2 +  __dtBusInfo[10], &__work2BUS[0].fltUpperLimit,  __dtBusInfo[11]);
        (void) memcpy(work2 +  __dtBusInfo[12], &__work2BUS[0].fltIntegPartK_1,  __dtBusInfo[13]);
        (void) memcpy(work2 +  __dtBusInfo[14], &__work2BUS[0].fltInK_1,  __dtBusInfo[15]);
        (void) memcpy(work2 +  __dtBusInfo[16], &__work2BUS[0].u16LimitFlag,  __dtBusInfo[17]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        AMCLIB_CurrentLoopPIp_w_FLT(*u1, __work1BUS, __work2BUS, __u2BUS, __u3BUS, *u4, *u5, *u6, *u7, *u8, *u9, *u10, *u11, *u12, *u13, __y1BUS, y2, y3, y4, y5, y6, work3, *p1, *p2);
